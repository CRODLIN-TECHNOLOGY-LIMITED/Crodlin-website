...
function MainInput() {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-4">
      <motion.div
        className="relative overflow-hidden backdrop-blur-lg shadow-xl border border-purple-500/10 rounded-2xl"
      ...
      >
        ...
        <div className="p-8 relative z-10">
          ...
          >
            <div className="inline-flex items-center px-4 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-300 mb-4">
              <Sparkles className="h-3.5 w-3.5 mr-2" />
              <span className="text-sm font-medium">AI-Powered Learning</span>
            </div>
            <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-300 to-indigo-300 mb-2 font-heading">Discover Your Learning Path</h2>
            <p className="text-gray-400 font-sans">Tell us what you want to learn, and we'll create a personalized roadmap for you</p>
          </motion.div>

          <form onSubmit={onSubmit} className="flex flex-col md:flex-row gap-4 relative z-10">
            <div className="flex-1 relative group">
              <div className={`absolute inset-0 rounded-xl transition-opacity duration-300 ${inputFocused ? 'opacity-100' : 'opacity-0'}`}>
                <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500/20 via-blue-500/20 to-indigo-500/20 blur-md"/>
              </div>
              <div className="relative flex items-center">
                <div className="absolute left-4">
                  <Search className={`w-5 h-5 transition-colors duration-300 ${inputFocused ? 'text-purple-400' : 'text-gray-500'}`} />
                </div>
                <input
                  type="text"
                  ...
                  className="w-full bg-[#0f0f13]/90 text-white placeholder-gray-400 border border-[#2a2a33] focus:border-purple-500/50 rounded-xl pl-12 pr-4 py-4 focus:outline-none focus:ring-1 focus:ring-purple-500/50 transition-all duration-300 font-sans shadow-inner shadow-black/20"
                />
              </div>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className={`bg-gradient-to-r from-[#4c0d52] to-[#3a0d94] hover:from-[#5c1262] hover:to-[#4a1da4] text-white px-8 py-4 rounded-xl border border-[#6c2972]/30 transition-all duration-300 font-medium shadow-lg shadow-purple-900/20 font-sans ${
          ...
          >
            <div className="flex items-center mb-4">
              ...
              <p className="text-gray-400 text-sm px-3 font-sans">Popular topics</p>
              <div className="flex-grow h-px bg-gradient-to-r from-transparent via-gray-700/50 to-transparent"></div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              {popularTopics.map(topic => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="px-4 py-2.5 bg-[#1a1a24]/70 hover:bg-[#2a2a38]/50 border border-[#2a2a33]/50 hover:border-purple-500/30 rounded-lg text-sm text-gray-300 hover:text-white transition-all duration-200 flex items-center gap-2 group font-sans backdrop-blur-md"
                >
                  <span className="p-1.5 rounded-md bg-purple-900/30 text-purple-300 group-hover:scale-110 transition-transform">
                    {topic.icon}
                  </span>
                  {topic.name}
                </motion.button>
      ...
      {roadmapData && !loading && (
        <motion.div 
          className="mt-12 w-full p-6 rounded-xl bg-black/30 border border-purple-500/10 backdrop-blur-sm"
